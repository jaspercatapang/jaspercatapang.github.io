...
const HuggingFaceIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" fillRule="evenodd" aria-hidden>
...
const PUBLICATION_CARDS = [
  ...
  { category: 'pending', title: 'ChatGPT as a Tool in Describing Variation and Change in English Worldwide', authors: 'Catapang, J.K.', monthYear: 'In press', venue: '—', citation: 'Catapang, J.K. (in press). <em>ChatGPT as a Tool in Describing Variation and Change in English Worldwide</em>.', pdfUrl: '', tags: ['NLP', 'World Englishes'] },
  ...
  { category: 'pending', title: 'When Image and Text Disagree: Cross-Modal Evidence Conflict in Multimodal Retrieval-Augmented Generation', authors: 'Catapang, J.K.', monthYear: 'Submitted', venue: '—', citation: 'Catapang, J.K. (submitted). <em>When Image and Text Disagree: Cross-Modal Evidence Conflict in Multimodal Retrieval-Augmented Generation</em>.', pdfUrl: '', tags: ['LLMs', 'NLP', 'machine learning', 'vision-language'] },
  ...
  { category: 'conference', title: 'Topic Modeling, Clade-assisted Sentiment Analysis, and Vaccine Brand Reputation Analysis of COVID-19 Vaccine-related Facebook Comments in the Philippines', authors: 'Catapang, J.K., & Cleofas, J.V.', monthYear: 'January 2022', venue: 'IEEE ICSC 2022 · Laguna Hills, CA, USA', citation: 'Catapang, J.K., & Cleofas, J.V. (January 2022). <em>Topic Modeling, Clade-assisted Sentiment Analysis, and Vaccine Brand Reputation Analysis of COVID-19 Vaccine-related Facebook Comments in the Philippines</em>. 2022 IEEE International Conference on Semantic Computing. Laguna Hills, California, USA. pp. 123-130. IEEE. <a href="https://ieeexplore.ieee.org/document/9736280" target="_blank" rel="noopener noreferrer" class="text-accent hover:underline">DOI: 10.1109/ICSC52841.2022.00026</a>. Acceptance rate: 20%.', pdfUrl: '/publications/topic-modeling-vaccine-facebook-icsc-2022.pdf', codeUrl: 'https://github.com/jcatapang/COVID-19-VERTEBRATE', tags: ['NLP', 'machine learning'] },
        ...
        <main id="main-content" className="flex-1 px-6 md:px-8 pb-10 max-w-4xl" tabIndex={-1}>
          ...
          <Section id="about" title="About">
            <div className="max-w-[38rem] space-y-4">
              <p>
                With over six years of experience in NLP and AI, I specialize in explainable AI (XAI), LLM post-training, and agentic evaluation—most notably leading the fine-tuning of the top-ranked GodziLLa-2 model (
          ...
          <Section id="publications" title="Publications" className="pb-10">
            ...
            <div className="space-y-3 mb-6">
              {PUBLICATION_CARDS.filter(p => p.category === 'pending' && matchesTagFilter(p)).map((pub, i) => (
                <PublicationCard
                  key={`pending-${i}`}
                  title={pub.title}
                  authors={pub.authors}
            ...
            <div className="space-y-3 mb-6">
              {PUBLICATION_CARDS.filter(p => p.category === 'journal' && matchesTagFilter(p)).map((pub, i) => (
                <PublicationCard
                  key={`journal-${i}`}
                  title={pub.title}
                  authors={pub.authors}
            ...
            <div className={`space-y-3 ${PUBLICATION_CARDS.some((p) => p.category === 'workshop') ? 'mb-6' : 'mb-10'}`}>
              {PUBLICATION_CARDS.filter(p => p.category === 'conference' && matchesTagFilter(p)).map((pub, i) => (
                <PublicationCard
                  key={`conference-${i}`}
                  title={pub.title}
                  authors={pub.authors}
            ...
            {PUBLICATION_CARDS.some((p) => p.category === 'workshop') && (
              <>
                <SubsectionTitle>Workshop Proceedings</SubsectionTitle>
                <div className="space-y-3 mb-10">
                  {PUBLICATION_CARDS.filter((p) => p.category === 'workshop' && matchesTagFilter(p)).map((pub, i) => (
                    <PublicationCard
                      key={`workshop-${i}`}
                      title={pub.title}
                      authors={pub.authors}
            ...
            <ul className="pl-5 space-y-1.5 text-sm">
              <li>Eskwelabs Data Science Sprint Judging. (April 18, 2026). Eskwelabs.</li>
